...
numpy.random.seed(7)
# load the dataset
dataframe = read_csv('train_format_up_and_down_23_stations.csv',header=None)
#print dataframe.columns.values
dataframe = dataframe.loc[dataframe[69]!=-1.0]
...
model = Sequential()
model.add(LSTM(6, input_shape=(1,trainX.shape[2])))
model.add(Dense(1))
...
# numpy.save('trainPredict', trainPredict)
# numpy.save('testPredict', testPredict)

# train_from_file = numpy.load('trainPredict')
# test_from_file = numpy.load('testPredict')

# print "saved file"
# print train_from_file.shape
# print test_from_file.shape
...
# plt.plot(trainY, label='Original training data')
# plt.plot(trainPredictPlot, label= 'Predicted training data')
plt.title('Upstream and Downstream - location and time')
plt.plot(testY,label='Original test data')
plt.plot(testPredictPlot,label='Predicted test data')
plt.ylabel('Speed in mph')
plt.xlabel('No of instances cumulatively processed')
plt.legend()
plt.show()
